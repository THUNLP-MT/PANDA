...
import sys
from data_utils import add_current_place, add_current_objects, sanitizeStr, formalize_action, recover_action
from data_utils import compose_instance_v1, compose_instance_v1_1, compose_instance_v2, compose_instance_v3, compose_instance_v4, compose_instance_v5
from eval_utils import load_model, findValidActionNewExploration, load_variation, get_model_output
...
def eval(args, task_num, logger):
    ...
    scores = []
    actions_samples = {task_num:dict()}
    for variation in variations:
        actions_samples[task_num][variation] = []
        # train_data = []
        ...
        while not done:
            ...
            actions = findValidActionNewExploration(predStrs, env, info['look'], recent_actions, sbert_model, logger) 
            action = actions[0]
            actions_samples[task_num][variation].append(actions)
        ...
        time.sleep(2)
    with open(os.path.join(args['output_path'], f'task{task_num}_action_samples.json'), "w") as json_file:
        json.dump(actions_samples, json_file)
...
def parse_args():
    ...
    if not debug:
        ...
        parser.add_argument("--sbert_path", type=str, default="./sentence-transformers_paraphrase-MiniLM-L6-v2")
        parser.add_argument("--local_llm", type=str, default="")
    else:
        parser.add_argument("--jar_path", type=str) 
